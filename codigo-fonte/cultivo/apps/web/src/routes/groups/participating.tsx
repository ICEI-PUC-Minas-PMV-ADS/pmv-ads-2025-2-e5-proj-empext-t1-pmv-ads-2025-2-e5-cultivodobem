...
export const Route = createFileRoute('/groups/participating')({ component: RouteComponent })
...
function RouteComponent() {
  ...
  const myGroups = (groups ?? []).filter((g: any) => (g.participants ?? []).some((p: any) => String(p) === String(userId)))

  return (
    <div className="p-4 max-w-3xl mx-auto">
      <h1 className="text-2xl font-semibold mb-4">Grupos que participo</h1>
      {!groups && <div>Carregando...</div>}
      {groups && myGroups.length === 0 && <div>Você ainda não participa de nenhum grupo.</div>}
