...
// Get the group that the user is a participant of
export const getUserGroup = query({
  ...
  handler: async (ctx, { userId }) => {
    ...
    const participantsFull = await fetchParticipants(
      ctx,
      userGroup.participants ?? []
    );
    return { ...userGroup, participantsFull };
  },
});
    ...
    if (String(g.createdBy) !== String(userId)) {
      try {
        await ctx.scheduler.runAfter(
          0,
          (internal as any).group.sendJoinNotification,
          {
        ...
        console.log(
          `Join notification scheduled for group creator ${g.createdBy}`
        );
      } catch (error) {
...
// User leaves the group voluntarily
export const leaveGroup = mutation({
  ...
  handler: async (ctx, { groupId, userId }) => {
    ...
    if (String(g.createdBy) === String(userId)) {
      throw new Error(
        "O criador do grupo não pode sair. Você deve encerrar o grupo."
      );
    }
...
export const sendJoinNotification: any = internalAction({
  ...
  handler: async (
    ctx,
    { creatorId, joiningUserId, joiningUserName, groupId, groupName }
  ): Promise<any> => {
    console.log(
      `Sending join notification to creator ${creatorId} for user ${joiningUserName} joining group ${groupName}`
    );
